...
var tableView = cc.Class({
    ...
    _initTableView: function (paramCount) {
        if (this._cellPool) {
            this.clear();
        }
        ...
        var groupCellCount = this._getGroupCellCount();

        this._count = Math.ceil(paramCount / groupCellCount);
    ...
    initTableView: function (paramCount, data) {
        ...
        // 存在Widget则在立即对齐
        var nodeW = this.node.getComponent(cc.Widget);
        var viewW = this._view.getComponent(cc.Widget);
        var contentW = this.content.getComponent(cc.Widget);
        if (nodeW) {
            nodeW.updateAlignment();
        }
        if (viewW) {
            viewW.updateAlignment();
        }
        if (contentW) {
            contentW.updateAlignment();
        }
        this._initTableView(paramCount);
    },

    selectCell: function (group) {
        return 0;
    },
    //*************************************************重写ScrollView方法*************************************************//
    // touch event handler
